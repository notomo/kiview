...
function! kiview#renamer#new(source_id, bufnr) abort
    ...
    function! renamer.open(path, items) abort
        ...
        let line = 1
        for item in a:items
            call nvim_buf_set_extmark(self.bufnr, s:namespace, line, 0, {})
    ...
    function! renamer.write() abort
        ...
        for line in lines
            ...
            call add(targets, {'from': item.path, 'to': line, 'id': marks[0][0], 'is_copy': item.is_copy})
        endfor
    ...
    function! renamer.complete(items, has_error) abort
        for item in a:items
